...
ARGUMENTS = [
    ...
    DeclareLaunchArgument('keepout', default_value='false',
                          description='whether or not a keepout filter is running already.' \
                          'Note this is the only way to do it currently.')
]
...
def generate_launch_description():
    ...
    # Nav2
    if not LaunchConfiguration('keepout'):
        ...
        nav2 = IncludeLaunchDescription(
            ...
            launch_arguments=[
                ('namespace', namespace),
                ('use_sim_time', use_sim_time),
                ('params_file', PathJoinSubstitution([pkg_turtlebot4_navigation, 'config', 'nav2_with_keepout.yaml'])),
            ],
            condition=IfCondition(LaunchConfiguration('nav2'))
        )
